...
void split(const std::string &s, const std::string &delimiter, 
        std::pair<std::string, std::string> &result) {
   if(delimiter.size() == 0){
    throw std::logic_error("[ERROR]: delimiter is empty\n");
   }
   if(s.size() == 0){
    throw std::logic_error("[ERROR]: string s size is empty\n");
   }
...
template <typename T = std::string>
void split(const std::string &s, const std::string &delimiters,
           std::vector<T> &result) {
...
bool Cal2tex::parse_ics() {
  ...
  /*
      ...
      if(file_content.empty()){
          std::cerr << "[ERROR]: File  " << m_ifname << " file_content is
     empty\n"; std::exit(1);
      }
      icalcomponent *calendar = icalparser_parse_string(file_content.c_str());
      if(!calendar){
          std::cerr << "[ERROR]: Could not parse string file_content into ical
     component\n"; std::exit(1);
      }
      for(icalcomponent *event = icalcomponent_get_first_component(calendar,
                                                                   ICAL_VEVENT_COMPONENT);
     event != nullptr; event = icalcomponent_get_next_component(calendar,
     ICAL_VEVENT_COMPONENT)){ Cal_events e; current_event =
     icalcomponent_get_summary(event); e.summary =    IS_CURRENT_NULLPTR
          current_event =
     icaltime_as_ical_string(icalcomponent_get_dtstart(event)); e.date_start =
     IS_CURRENT_NULLPTR current_event =
     icaltime_as_ical_string(icalcomponent_get_dtend(event)); e.date_end  =
     IS_CURRENT_NULLPTR current_event = icalcomponent_get_location(event);
